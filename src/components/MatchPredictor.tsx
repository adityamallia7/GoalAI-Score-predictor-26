...
export default function MatchPredictor({ teams, onPredictionSaved }: MatchPredictorProps) {
  ...
  return (
    <div className="space-y-8" id="match-predictor-section">
      ...
      {/* Side-by-Side Comparison before prediction */}
      {!prediction && !loading && (
        <div className="bg-carbon-card border border-white/5 p-6 rounded-2xl space-y-6 shadow-2xl relative" id="attributes-comparison">
          ...
          <h3 className="text-xs font-mono font-extrabold text-white uppercase tracking-wider flex items-center gap-2">
            <BarChart2 className="w-4 h-4 text-emerald-400" /> Pre-match Ratings Matrix Comparison
          </h3>
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="text-center mb-1">
                <span className="text-[10px] text-slate-400 uppercase tracking-widest font-black font-outfit">Squad Quality (Overall)</span>
              </div>
              <div className="flex justify-between text-xs font-mono mb-1.5 font-bold">
                <span className="flex items-center gap-1.5 text-slate-200 max-w-[45%] min-w-0 truncate">
                  <TeamFlag name={teamA.name} height={12} className="flex-shrink-0" />
                  <span className="truncate">{teamA.name}</span>
                  <span className="text-[#A3E635] font-bold">({teamA.overall})</span>
                </span>
                <span className="flex items-center gap-1.5 text-slate-200 max-w-[45%] min-w-0 truncate justify-end">
                  <span className="text-[#06B6D4] font-bold">({teamB.overall})</span>
                  <span className="truncate">{teamB.name}</span>
                  <TeamFlag name={teamB.name} height={12} className="flex-shrink-0" />
                </span>
              </div>
              <div className="h-2 bg-zinc-950 rounded-full overflow-hidden flex border border-white/5">
                <div 
                  className="bg-gradient-to-r from-emerald-500 to-teal-500 h-full transition-all duration-300" 
                  style={{ width: getComparisonWidths(teamA.overall, teamB.overall).widthA }}
                />
                <div 
                  className="bg-zinc-800 h-full transition-all duration-300" 
                  style={{ width: getComparisonWidths(teamA.overall, teamB.overall).widthB }}
                />
              </div>
            </div>
            ...
            <div>
              <div className="text-center mb-1">
                <span className="text-[10px] text-slate-400 uppercase tracking-widest font-black font-outfit">Attacking Prowess</span>
              </div>
              <div className="flex justify-between text-xs font-mono mb-1.5 font-bold">
                <span className="text-[#A3E635] font-bold flex items-center gap-1.5">
                  <TeamFlag name={teamA.name} height={12} className="flex-shrink-0" /> {teamA.attack.toFixed(2)}
                </span>
                <span className="text-[#06B6D4] font-bold flex items-center gap-1.5">
                  {teamB.attack.toFixed(2)} <TeamFlag name={teamB.name} height={12} className="flex-shrink-0" />
                </span>
              </div>
              <div className="h-2 bg-zinc-950 rounded-full flex overflow-hidden border border-white/5">
                <div 
                  className="bg-gradient-to-r from-emerald-500 to-emerald-450 h-full transition-all duration-300" 
                  style={{ width: getComparisonWidths(teamA.attack, teamB.attack).widthA }}
                />
                <div 
                  className="bg-zinc-800 h-full transition-all duration-300" 
                  style={{ width: getComparisonWidths(teamA.attack, teamB.attack).widthB }}
                />
              </div>
            </div>
            ...
            <div>
              <div className="text-center mb-1">
                <span className="text-[10px] text-slate-400 uppercase tracking-widest font-black font-outfit font-bold">Conceding Weakness <span className="text-[8px] text-slate-500 lowercase">(lower = better)</span></span>
              </div>
              <div className="flex justify-between text-xs font-mono mb-1.5 font-bold">
                <span className="text-[#A3E635] font-bold flex items-center gap-1.5">
                  <TeamFlag name={teamA.name} height={12} className="flex-shrink-0" /> {teamA.defense.toFixed(2)}
                </span>
                <span className="text-[#06B6D4] font-bold flex items-center gap-1.5">
                  {teamB.defense.toFixed(2)} <TeamFlag name={teamB.name} height={12} className="flex-shrink-0" />
                </span>
              </div>
              <div className="h-2 bg-zinc-950 rounded-full flex overflow-hidden border border-white/5">
                <div 
                  className="bg-gradient-to-r from-emerald-500 to-emerald-450 h-full transition-all duration-300" 
                  style={{ width: getComparisonWidths(teamA.defense, teamB.defense).widthA }}
                />
                <div 
                  className="bg-zinc-800 h-full transition-all duration-300" 
                  style={{ width: getComparisonWidths(teamA.defense, teamB.defense).widthB }}
                />
              </div>
            </div>

            {/* Form sequence */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8 pt-4 border-t border-white/5">
              <div className="text-center font-mono">
                <div className="text-[10px] text-slate-400 uppercase mb-2 flex items-center justify-center gap-1 font-bold">
                  <Flame className="w-3.5 h-3.5 text-red-400" /> {teamA.name} Qualification Form
                </div>
                <div className="flex justify-center gap-1.5">
                  {teamA.form.map((f, idx) => (
                    <span key={idx} className={`w-7 h-7 rounded-lg flex items-center justify-center font-black text-xs ${f === 'W' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/35' : f === 'D' ? 'bg-zinc-800/40 text-slate-350 border border-zinc-850' : 'bg-red-500/15 text-red-400 border border-red-500/20'}`}>
                      {f}
                    </span>
                  ))}
                </div>
              ...
              <div className="text-center font-mono">
                <div className="text-[10px] text-slate-400 uppercase mb-2 flex items-center justify-center gap-1 font-bold">
                  <Flame className="w-3.5 h-3.5 text-red-400" /> {teamB.name} Qualification Form
                </div>
                <div className="flex justify-center gap-1.5">
                  {teamB.form.map((f, idx) => (
                    <span key={idx} className={`w-7 h-7 rounded-lg flex items-center justify-center font-black text-xs ${f === 'W' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/35' : f === 'D' ? 'bg-zinc-800/40 text-slate-350 border border-zinc-850' : 'bg-red-500/15 text-red-400 border border-red-500/20'}`}>
                      {f}
                    </span>
                  ))}
